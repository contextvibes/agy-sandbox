...
9. **APFS Block-Level Compaction & Hole-Punching:** Standard `rsync -S` or block copies can bloat paravirtualized sparse images by dirtying unused segments. To shrink images, always execute a guest zero-fill sweep, then use Swift's `F_PUNCHHOLE` `fcntl` hole puncher on the host while the image is completely unlocked (VM offline).
