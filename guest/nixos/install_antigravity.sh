#!/usr/bin/env bash
# Antigravity Linux / NixOS Native Installer
# Installs and updates Google Antigravity 2.0 and Antigravity IDE natively on Linux.
# Resolves the latest official Google tarballs from https://antigravity.google/download.
# Features a localized, network-independent caching layer to bypass network downloads.
...
# Candidate local directories to scan for cached/pre-downloaded tarballs
LOCAL_DOWNLOADS_DIRS=(
  "/home/nixos/shared/downloads"
  "$(dirname "${BASH_SOURCE[0]:-$0}")/../../downloads"
  "./downloads"
  "/var/tmp"
  "."
)
...
Usage:
  install_antigravity.sh [install|update] [options]
  install_antigravity.sh --status
  install_antigravity.sh --print-downloads
  install_antigravity.sh --uninstall
...
Options:
  ...
  --install-url URL      Store custom URL used by the antigravity-linux update command
...
Caching Feature:
  This script will automatically check local 'downloads/' folders (including mounted host
  directories inside NixOS) for pre-downloaded tarballs (e.g. Antigravity.tar.gz) matching
  the latest version. If found, it bypasses network downloads.
...
require_root_or_reexec() {
  ...
  err "System-wide install needs root. Run with: sudo bash guest/nixos/install_antigravity.sh"
}

install_deps_debian() {
  [ "$INSTALL_DEPS" -eq 1 ] || return 0
  # Avoid running apt on NixOS, which has nix-ld and pre-configured packages
  if command -v apt-get >/dev/null 2>&1 && [ ! -f /etc/NIXOS ]; then
...
resolve_main_bundle() {
  ...
  python3 - "$html" "$DOWNLOAD_PAGE" <<'PY' > "$tmpdir/main_js_url.txt"
import re, sys
...
PY
  local main_js_url
  main_js_url=$(cat "$tmpdir/main_js_url.txt")
...
find_cached_file() {
  local product="$1"      # "desktop" or "ide"
  local version="$2"      # resolved latest version
  
  local name_patterns=()
  if [ "$product" = "desktop" ]; then
    name_patterns=("Antigravity-${version}.tar.gz" "Antigravity.tar.gz")
  elif [ "$product" = "ide" ]; then
    name_patterns=(
      "Antigravity-IDE-${version}.tar.gz"
      "Antigravity IDE-${version}.tar.gz"
  ...
  for dir in "${LOCAL_DOWNLOADS_DIRS[@]}"; do
    if [ -d "$dir" ]; then
      for pattern in "${name_patterns[@]}"; do
        local filepath="$dir/$pattern"
        if [ -f "$filepath" ]; then
          printf '%s\n' "$filepath"
          return 0
        fi
...
copy_to_user_desktop() {
  ...
  if [ -d "$user_home/Desktop" ]; then
    local dest="$user_home/Desktop/$filename"
    cp -f "$desktop_file" "$dest"
    chown "$target_user":users "$dest" 2>/dev/null || chown "$target_user" "$dest" 2>/dev/null || true
    chmod +x "$dest"
    log "Created Desktop shortcut for $target_user at $dest"
  fi
}
...
install_desktop_app() {
  ...
  local cached_file
  if cached_file=$(find_cached_file desktop "$version"); then
    log "Found cached local Antigravity 2.0 package: $cached_file"
    cp "$cached_file" "$archive"
  else
...
#!/usr/bin/env bash
# Antigravity Wrapper
LOCK_FILE="/home/nixos/.config/Antigravity/SingletonLock"
if [ -L "\$LOCK_FILE" ]; then
    TARGET=\$(readlink "\$LOCK_FILE" || true)
    PID=\$(echo "\$TARGET" | grep -o -E '[0-9]+\$' || true)
    if [ -n "\$PID" ] && ! ps -p "\$PID" >/dev/null 2>&1; then
        rm -f "/home/nixos/.config/Antigravity/SingletonLock" \\
              "/home/nixos/.config/Antigravity/SingletonCookie" \\
              "/home/nixos/.config/Antigravity/SingletonSocket"
    fi
fi
exec "$root/$top_dir/antigravity" "\$@"
EOF
...
Exec=/usr/local/bin/antigravity --no-sandbox --disable-gpu %U
Icon=antigravity
Terminal=false
Type=Application
...
DESKTOP
  copy_to_user_desktop /usr/share/applications/antigravity.desktop
  refresh_desktop_caches
...
install_ide_app() {
  ...
  local cached_file
  if cached_file=$(find_cached_file ide "$version"); then
    log "Found cached local Antigravity IDE package: $cached_file"
    cp "$cached_file" "$archive"
  else
  ...
  cat > /usr/local/bin/antigravity-ide <<EOF
#!/usr/bin/env bash
# Antigravity IDE Wrapper
LOCK_FILE="/home/nixos/.config/Antigravity IDE/SingletonLock"
if [ -L "\$LOCK_FILE" ]; then
    TARGET=\$(readlink "\$LOCK_FILE" || true)
    PID=\$(echo "\$TARGET" | grep -o -E '[0-9]+\$' || true)
    if [ -n "\$PID" ] && ! ps -p "\$PID" >/dev/null 2>&1; then
        rm -f "/home/nixos/.config/Antigravity IDE/SingletonLock" \\
              "/home/nixos/.config/Antigravity IDE/SingletonCookie" \\
              "/home/nixos/.config/Antigravity IDE/SingletonSocket"
    fi
fi
exec "$root/$install_dir/antigravity-ide" "\$@"
EOF
...
Exec=/usr/local/bin/antigravity-ide --no-sandbox --disable-gpu %F
...
DESKTOP
  copy_to_user_desktop /usr/share/applications/antigravity-ide.desktop
  refresh_desktop_caches
...
install_manager_command() {
  ...
  local local_script_path="/home/nixos/shared/guest/nixos/install_antigravity.sh"
  
  # Try to resolve our real absolute path on the host-to-guest share if available
  local real_path
  real_path=$(readlink -f "${BASH_SOURCE[0]:-$0}" 2>/dev/null || realpath "${BASH_SOURCE[0]:-$0}" 2>/dev/null || echo "")
  if [ -x "$real_path" ]; then
    local_script_path="$real_path"
  fi
...
if [ -f "\$LOCAL_PATH" ] && [ -x "\$LOCAL_PATH" ]; then
  # Local script is available natively! Run it directly (saving internet and avoiding opensnap downloads)
  if [ "\$(id -u)" -eq 0 ]; then
    exec bash "\$LOCAL_PATH" "\$@"
  else
    exec sudo -E env "ANTIGRAVITY_LINUX_INSTALLER_URL=\$SCRIPT_URL" bash "\$LOCAL_PATH" "\$@"
  fi
elif [ -n "\$SCRIPT_URL" ]; then
  # Fallback to stored helper URL
  if [ "\$(id -u)" -eq 0 ]; then
  ...
  echo "ERROR: Local installer script missing at \$LOCAL_PATH and no fallback update URL is configured." >&2
  exit 1
fi
SH
...
#!/usr/bin/env bash
exec /usr/local/bin/antigravity-linux update --desktop "$@"
SH
  chmod +x /usr/local/bin/update-antigravity

  cat > /usr/local/bin/update-antigravity-ide <<'SH'
#!/usr/bin/env bash
exec /usr/local/bin/antigravity-linux update --ide "$@"
SH
